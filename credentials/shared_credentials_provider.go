...
func loadProfile(filename, profile string) (Value, error) {
	...
	id, ok := iniProfile["access_key"]
	if !ok {
		return Value{}, apierr.New("SharedCredsAccessKey",
			fmt.Sprintf("shared credentials %s in %s did not contain access_key", profile, filename),
			nil)
	}

	secret, ok := iniProfile["secret_key"]
	if !ok {
		return Value{}, apierr.New("SharedCredsSecret",
			fmt.Sprintf("shared credentials %s in %s did not contain secret_key", profile, filename),
			nil)
	}
